...
pub(crate) fn detect_pre_body(status: u16, headers: &HeaderMap) -> Option<UncertainReason> {
    ...
    if status == 403 && server_is_cloudfront(headers) {
        return Some(UncertainReason::CloudfrontChallenge);
    }
    None
}
...
fn server_is_cloudfront(headers: &HeaderMap) -> bool {
    headers
        .get("server")
        .and_then(|v| v.to_str().ok())
        .is_some_and(|s| s.eq_ignore_ascii_case("cloudfront"))
        || headers
            .get("x-cache")
            .and_then(|v| v.to_str().ok())
            .is_some_and(|s| s.to_ascii_lowercase().contains("cloudfront"))
}
...
mod tests {
    ...
    #[test]
    fn pre_body_flags_cloudfront_403() {
        assert_eq!(
            detect_pre_body(403, &headers(&[("server", "CloudFront")])),
            Some(UncertainReason::CloudfrontChallenge)
        );
    }
