...
`npm run unit` covers store/action behavior without a browser.
`npm run smoke` starts the Vite dev server through Playwright and covers
the home/catalog path plus a cold `#/scan/:id` snapshot route. Use these
for SPA routing, store/action, and component changes; use the Rust
workspace checks above for server/API changes.
