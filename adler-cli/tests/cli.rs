...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn doctor_pinterest_oembed_fixture_is_healthy() {
    let server = MockServer::start().await;
    Mock::given(any())
        .and(path("/oembed.json"))
        .and(query_param("url", "https://www.pinterest.com/blue/"))
        .respond_with(ResponseTemplate::new(200).set_body_string(
            r#"{"provider_name":"Pinterest","author_url":"https://www.pinterest.com/blue/"}"#,
        ))
        .mount(&server)
        .await;
    Mock::given(any())
    ...
    let json = format!(
        r#"{{"sites":[{{
            "name":"Pinterest",
            "url":"{0}/oembed.json?url=https://www.pinterest.com/{{username}}/",
            "signals":[
                {{"kind":"status_found","codes":[200]}},
                {{"kind":"body_username","text":"\"author_url\":\"https://www.pinterest.com/{{username}}/\""}},
                {{"kind":"status_not_found","codes":[404]}}
            ],
            "known_present":"blue"
        }}]}}"#,
    ...
    let stdout = String::from_utf8(assert.get_output().stdout.clone()).unwrap();
    let parsed: serde_json::Value = serde_json::from_str(&stdout).expect("valid JSON envelope");
    assert_eq!(parsed["sites"][0]["name"], "Pinterest");
    assert_eq!(parsed["sites"][0]["verdict"], "healthy");
    assert!(parsed["sites"][0]["issues"].as_array().unwrap().is_empty());
    assert_eq!(parsed["summary"]["healthy"], 1);
    assert_eq!(parsed["summary"]["failing"], 0);
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn doctor_patreon_status_fixture_is_healthy() {
    let server = MockServer::start().await;
    Mock::given(any())
        .and(path("/blue"))
        .respond_with(ResponseTemplate::new(200).set_body_string(
            r#"<html><head><title>Patreon</title></head><body data-generic-shell="true"></body></html>"#,
        ))
        .mount(&server)
        .await;
    Mock::given(any())
    ...
    let json = format!(
        r#"{{"sites":[{{
            ...
            "known_present":"blue",
            "tags":["blog","bot-protected","creator"]
        }}]}}"#,
        server.uri()
    );
    let sites = sites_file(&json);
    ...
    let stdout = String::from_utf8(assert.get_output().stdout.clone()).unwrap();
    let parsed: serde_json::Value = serde_json::from_str(&stdout).expect("valid JSON envelope");
    assert_eq!(parsed["sites"][0]["name"], "Patreon");
    assert_eq!(parsed["sites"][0]["verdict"], "healthy");
    assert!(parsed["sites"][0]["issues"].as_array().unwrap().is_empty());
    assert_eq!(parsed["summary"]["healthy"], 1);
    assert_eq!(parsed["summary"]["failing"], 0);
...
async fn doctor_reddit_oauth_json_fixture_is_healthy_with_session() {
    let server = MockServer::start().await;
    Mock::given(any())
        .and(path("/user/reddit/about"))
        .and(wiremock::matchers::header("authorization", "Bearer token"))
        .respond_with(
            ResponseTemplate::new(200).set_body_string(r#"{"kind":"t2","data":{"name":"reddit"}}"#),
        )
        .mount(&server)
        .await;
    Mock::given(any())
    ...
    let json = format!(
        r#"{{"sites":[{{
            "name":"Reddit",
            "url":"{0}/user/{{username}}/about",
            "signals":[
                {{"kind":"status_found","codes":[200]}},
                {{"kind":"json_username","pointer":"/data/name"}},
                {{"kind":"status_not_found","codes":[404]}}
            ],
            "known_present":"reddit",
            "access":{{"session":"reddit"}}
        }}]}}"#,
        server.uri()
    );
    let sites = sites_file(&json);
    let mut sessions = NamedTempFile::new().expect("temp sessions");
    writeln!(sessions, "[reddit]\nAuthorization = \"Bearer token\"").expect("write sessions");
    let assert = adler()
        .args([
            "--sites",
            sites.path().to_str().unwrap(),
    ...
    let stdout = String::from_utf8(assert.get_output().stdout.clone()).unwrap();
    let parsed: serde_json::Value = serde_json::from_str(&stdout).expect("valid JSON envelope");
    assert_eq!(parsed["sites"][0]["name"], "Reddit");
    assert_eq!(parsed["sites"][0]["verdict"], "healthy");
    assert!(parsed["sites"][0]["issues"].as_array().unwrap().is_empty());
    assert_eq!(parsed["summary"]["healthy"], 1);
    assert_eq!(parsed["summary"]["failing"], 0);
